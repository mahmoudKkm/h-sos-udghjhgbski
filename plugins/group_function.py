...
########################################################################################################################
    ...
    if m.text == (get_db_botname() or "القائد"):
        texting = ["اؤمر " + (get_db_botname() or "القائد") + " شتريد؟❤️🥺",
                   ...
                   "قلبى بينادى على " + (get_db_botname() or "القائد") + "😘",
                   "نعسان محدش يصحينى🙄"
                   ]
        await m.reply_text(random.choice(texting))
        return

    if m.text == "بوت غادر" or m.text == (get_db_botname() or "القائد") + " غادر":
        if secsudo(m):
            try:
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [ InlineKeyboardButton ( "CH SOURCE", url=f"https://t.me/M_T_lI" )], 
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == "سفير":
        ...
        try:
            ...
            await c.send_message("7087517667", f"❤️╖ نداء لك ايها المبرمج\n📟╢ بواسطة {sender_name}\n📆╢ يوم *{today}*\n🕑╢ الساعه *{clock}*\n💌╢ اسم الجروب {name_chat}\n🔰╢ ايدي الجروب *{id_chat}*\n⚙️╢ عدد اعضاء الجروب *{num_member}*\n⛓╢ رابط المسج {message_link}\n🔍╜ الرابط {link_group}", parse_mode=enums.ParseMode.MARKDOWN)
        else:
